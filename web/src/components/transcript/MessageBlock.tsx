...
function MessageBlockInner({ msg, provider, toolResults, forceToolsOpen, showHeader = true }: {
  ...
  if (msg.role === 'user') {
    ...
    return (
      ...
      >
        <div className="mb-1 flex items-baseline gap-2 font-body text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: accent }}>
          <span className="g-prompt" /> You
        ...
        <ClampedBlock text={texts.map((b) => (b.kind === 'text' ? b.text : '')).join('\n')}>
          <div className="whitespace-pre-wrap break-words font-body text-[12.5px] leading-relaxed">
  ...
  return (
    <div className="py-0.5">
      {showHeader && (
        <div className="mb-1 flex items-baseline gap-2 pt-1.5 font-body text-[10px] font-semibold uppercase tracking-[0.14em] text-mut">
          <span className="g-agent" style={{ color: accent }} /> Assistant
          <span className="font-normal normal-case tracking-normal text-faint">{timeOf(msg.timestamp)}</span>
          {msg.model && <span className="font-body font-normal text-[color:var(--ansi-cyan)]">{msg.model.replace(/^claude-/, '')}</span>}
